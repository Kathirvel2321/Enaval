...
const PhaseOneEnding = ({ dialogue, dialogueIndex, onNextPhase }) => {
  ...
  const mainImage =
    dialogueIndex >= 21
      ? '/projectimages/saree2.png'
      : dialogueIndex >= 19
      ? '/projectimages/saree.png'
      : dialogueIndex >= 12
      ? '/projectimages/hi1.png'
      : dialogueIndex >= 5
      ? '/projectimages/schoolher.png'
      : dialogueIndex >= 3
      ? '/projectimages/hi1.png'
  ...
  const popImage =
    dialogueIndex >= 23
      ? '/projectimages/emotional.png'
      : dialogueIndex >= 22
      ? '/projectimages/unconcious.png'
      : dialogueIndex >= 19
      ...
      : dialogueIndex >= 6
      ? '/projectimages/popanswer.png'
      : dialogueIndex >= 5
      ? '/projectimages/popguess.png'
      : ''
  ...
  const dialogueCardClass = showModernRomanticTheme
    ? 'relative rounded-[1.4rem] border border-white/80 bg-white/74 px-4 py-4 shadow-[0_14px_26px_rgba(68,30,54,0.16)] backdrop-blur-xl sm:px-6 sm:py-5'
    : 'relative rounded-[1.4rem] border border-white/35 bg-black/48 px-4 py-4 shadow-[0_14px_26px_rgba(0,0,0,0.45)] backdrop-blur-md sm:px-6 sm:py-5'
  const dialogueTextClass = showModernRomanticTheme
    ? 'text-center text-[1.3rem] font-semibold leading-[1.2] text-[#6b2449] sm:text-[1.9rem]'
    : 'text-center text-[1.3rem] font-semibold leading-[1.2] text-rose-50 sm:text-[1.9rem]'
    ...
    >
      {onNextPhase ? (
        <button
          ...
          className="absolute right-3 top-3 z-[95] rounded-full border border-white/70 bg-white/85 px-3 py-1 text-xs font-semibold text-[#6b2449] shadow-[0_8px_20px_rgba(40,16,32,0.22)] backdrop-blur sm:right-4 sm:top-4"
        >
          Next Phase
        </button>
      ) : null}
      ...
      <AnimatePresence>
        {showSchoolBackdrop ? (
          <motion.img
            key="school-backdrop"
            src="/projectimages/schoolher.png"
            alt=""
      ...
      <div className="absolute bottom-5 left-1/2 z-20 w-[min(92vw,700px)] -translate-x-1/2 px-2 sm:bottom-8">
        <motion.div
