...
const PhaseThree = ({ dialogueIndex, onNextPhase }) => {
  const mainImage =
    dialogueIndex >= 11
      ? '/projectimages/smile2.png'
      : dialogueIndex >= 10
      ? '/projectimages/wait.png'
      : dialogueIndex >= 8
    ...
    >
      {onNextPhase ? (
        <button
          ...
          className="absolute right-3 top-3 z-[95] rounded-full border border-white/75 bg-white/86 px-3 py-1 text-xs font-semibold text-[#21445f] shadow-[0_8px_20px_rgba(20,44,68,0.2)] backdrop-blur sm:right-4 sm:top-4"
        >
          Next Phase
        </button>
      ) : null}
      ...
      <div className="absolute bottom-5 left-1/2 z-20 w-[min(92vw,700px)] -translate-x-1/2 px-2 sm:bottom-8">
        <motion.div
          ...
          className="relative rounded-[1.4rem] border border-white/80 bg-white/72 px-4 py-4 shadow-[0_14px_26px_rgba(20,44,68,0.18)] backdrop-blur-xl sm:px-6 sm:py-5"
        >
          <AnimatePresence mode="wait">
            <motion.p
              key={dialogueIndex}
              ...
              className="text-center text-[1.22rem] font-semibold leading-[1.2] text-[#21445f] sm:text-[1.78rem]"
