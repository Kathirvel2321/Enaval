...
const REVEAL_DIALOGUES = [
  ...
  'Intha kannu munnadi theriyura kaatchi ithu thaan.',
  ...
  'Ippo theriyutha ava enga paathutu irukka nu?',
  'She will watch left side.',
  'She will look right side.',
  'Ella pakkamum paapa, aana ennai thavira.',
  'Wait, I have a small doubt.',
  'Will you ever look at me... or not?',
  'Unmaiyava?',
  'At least ippo aavathu sonniye, athuve podhum.',
  'Athu therinja vishayam thaan…',
  'Engayo ava pakkattum…',
  "But look at her... isn't she just breathtaking?",
  'Ethu ve alagu nu ne sonnenga na… Athula enna ne soluveenga?',
  'Ungalukum pakanuma?',
  'Have you ever seen a peacock dance',
  'Like the magic of a peacock in the rain... that’s how beautiful my world is. ',
  'Pakerengala?',
  'See, I told you... the rain makes everything beautiful.',
  "But it's only because of her... that even the rain looks this special",
  'Alagaa Malaiya rasichikite irukapothu…',
  'Antha malai sound kulla…  Oru alaga kural enoda kathuku kekuthu…',
...
const ENDING_DIALOGUES = [
  'So, how was it? Breathtakingly beautiful, right?',
  'Ithula pakave ivlo alaga irukkaley...',
  'Just imagine my heart’s condition when I saw her in person.',
  'That first time I saw her... it’s still locked perfectly in my eyes., innum enoda kannukulla appadiye irukku.',
  'Enaku therinju ava...',
  'Manja color la dress, chudidhar.',
...
const Background = () => {
  const saved = useMemo(() => getSavedCheckpoint(), [])
  const restoredPhase =
    saved?.storyPhase === 'phase4'
      ? 'phase4'
      : saved?.storyPhase === 'phase3'
      ? 'phase3'
      : saved?.storyPhase === 'ending'
      ? 'ending'
      : saved?.storyPhase === 'reveal' || saved?.storyPhase === 'countdown' || saved?.storyPhase === 'transition'
      ? 'reveal'
      : 'idle'
  const [dialogueIndex, setDialogueIndex] = useState(() =>
    clampNumber(saved?.dialogueIndex, 0, DIALOGUES.length - 1, 0),
  )
  const [storyPhase, setStoryPhase] = useState(restoredPhase)
  const [countdownValue, setCountdownValue] = useState(5)
  const [revealDialogueIndex, setRevealDialogueIndex] = useState(() =>
    clampNumber(saved?.revealDialogueIndex, 0, REVEAL_DIALOGUES.length - 1, 0),
  )
  const [showWindowImage, setShowWindowImage] = useState(false)
  const [revealChoice, setRevealChoice] = useState(() =>
    saved?.revealChoice === 'papen' || saved?.revealChoice === 'illaya' ? saved.revealChoice : null,
  )
  const [showChoiceReady, setShowChoiceReady] = useState(false)
  const [showRain, setShowRain] = useState(false)
  const [endingDialogueIndex, setEndingDialogueIndex] = useState(() =>
    clampNumber(saved?.endingDialogueIndex, 0, ENDING_DIALOGUES.length - 1, 0),
  )
  const [phaseThreeDialogueIndex, setPhaseThreeDialogueIndex] = useState(() =>
    clampNumber(saved?.phaseThreeDialogueIndex, 0, PHASE_THREE_DIALOGUES.length - 1, 0),
  )
  ...
  const skipInitialCountdownRef = useRef(
    restoredPhase === 'reveal' || restoredPhase === 'ending' || restoredPhase === 'phase3' || restoredPhase === 'phase4',
  )
  ...
  }, [dialogueIndex, endingDialogueIndex, phaseFourDialogueIndex, phaseThreeDialogueIndex, revealChoice, revealDialogueIndex, storyPhase])
  ...
  const characterImage = dialogueIndex >= 2 ? '/projectimages/hi1.png' : '/projectimages/hi.png'
  ...
  const peekImage =
    revealDialogueIndex >= SING2_INDEX
      ...
      ? '/projectimages/popmelted.png'
      : revealChoice === 'papen' && revealDialogueIndex >= 12
      ? '/projectimages/popshock.png'
      : revealChoice === 'illaya' && revealDialogueIndex >= 14
      ? '/projectimages/poppredict.png'
      : revealDialogueIndex >= 10
      ? '/projectimages/popmessage2.png'
      : revealDialogueIndex >= 6
      ? '/projectimages/popsmile.png'
      ...
      <section className="relative z-10 min-h-screen overflow-hidden" onPointerDown={handleScreenTap}>
        {!showPhaseOneEnding && !showPhaseThree && !showPhaseFour ? (
          <button
            ...
            className="absolute right-3 top-3 z-[95] rounded-full border border-white/75 bg-white/88 px-3 py-1 text-xs font-semibold text-rose-700 shadow-[0_8px_20px_rgba(62,23,49,0.2)] backdrop-blur sm:right-4 sm:top-4"
          >
            Next Phase
          </button>
        ) : null}
        ...
        <AnimatePresence>
          {showStoryReveal && (
            ...
            >
              <motion.img
                aria-hidden
                src="/projectimages/building.png"
              ...
              <div className="absolute bottom-4 left-1/2 z-30 w-[min(92vw,640px)] -translate-x-1/2 sm:bottom-6 lg:bottom-7 lg:w-[min(42vw,520px)]">
                <motion.div
                  initial={{ opacity: 0, y: 20, scale: 0.98 }}
                  animate={{ opacity: 1, y: 0, scale: 1 }}
                  className="relative overflow-hidden rounded-[1.55rem] border border-white/55 bg-gradient-to-br from-[#fff9fc]/90 via-[#ffeef6]/84 to-[#fff7fb]/88 px-4 py-4 shadow-[0_14px_28px_rgba(33,10,37,0.24)] ring-1 ring-rose-200/45 backdrop-blur-xl sm:px-6 sm:py-5 lg:rounded-[1.45rem] lg:px-5 lg:py-4 lg:shadow-[0_14px_30px_rgba(33,10,37,0.22)]"
                >
                  <motion.div
                    aria-hidden
                  ...
                  <div className="relative min-h-[82px] sm:min-h-[96px] lg:min-h-[78px]">
                    <AnimatePresence mode="wait">
                      <motion.h2
                        ...
                        exit={{ opacity: 0, y: -8, filter: 'blur(5px)' }}
                        transition={{ duration: 0.45, ease: 'easeOut' }}
                        className="bg-gradient-to-r from-[#6f1043] via-[#b52f78] to-[#6f1043] bg-clip-text text-[1.36rem] font-semibold leading-[1.16] text-transparent drop-shadow-[0_2px_8px_rgba(182,82,130,0.3)] sm:text-[1.95rem] lg:text-[1.5rem] lg:leading-[1.18]"
                        style={{ fontFamily: "'Cormorant Garamond', serif" }}
                      >
        ...
        <AnimatePresence>{showPhaseFour && <PhaseFour dialogueIndex={phaseFourDialogueIndex} />}</AnimatePresence>
      </section>
    </main>
  )
}
