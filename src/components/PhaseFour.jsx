...
const PHASE_FIVE_DIALOGUES = [
  ...
  'Athu ethuke na kekanum, athu thana soluva thereyum.',
  'Analum na ne solleren.',
  'My dreams aren’t just one or two... they are endless.',
  'Aana ella aasaigallum neeye irukka.',
  'Antha edathula yarayum ennala nenaithu kuda pakka mudeyala.',
  'Enaikachi Oru nal ...',
  'Morning illama, night illama, oru evening time pola...',
  'Apade vanatha patha orange color la amaithiye ya erukum.',
  'While the whole world is silent...',
  'Unnai enoda scooty la vachu... just you and me ',
  'Apadeye scooty la poykede eruka pothu. kathaipoma padu kedekeday.',
  ...
  'Meene pinna poi palakam kedayathu.',
  'Vali kaadiyaga ne varuvaya?',
]
const PHASE_SIX_DIALOGUES = [
  'Enoda Alagee...',
  'Ipo na ethu pannathu onaku en mela love vara vaikanum apade la oru intention la na satheyama pannala.',
  'Etho unaku pudekumey nu panen.',
  'Ethu panurathuke na efforts la poda kasta paderken Kaushi, onakake apade elam scene la poda maden na.',
  'Just unakage na panen.ama enaku oru doubt paa.',
  'Ena oru time yachi love panerekaya?',
]
const PHASE_SIX_AFTER_Q1 = ['Ithye mari kelvi thana first website la unkeda kedain.']
const PHASE_SIX_YES_DIALOGUES = ['Apo ethuke paa ne evlo nal yen enkeda sollala ne.', 'Vanthu sollalam la.', 'Enaku varutham paa.']
...
const ENDING_NOTE_KEY = 'love-story-ending-note-v1'
...
const PhaseFour = ({ onNextPhase }) => {
  ...
  useEffect(() => {
    ...
    const saved = window.localStorage.getItem(ENDING_NOTE_KEY)
    if (saved) setEndingInput(saved)
  }, [])
  ...
  const getPoints = () => {
    ...
    return {
      left: { x: w * 0.33, y: h * 0.63 },
      right: { x: w * 0.67, y: h * 0.57 },
      mid: { x: w * 0.5, y: h * 0.58 },
    }
  }
  ...
  const roundOne = async () => {
    ...
    setLeftPose('bathit1')
    await sleep(220)
    await animateShuttle({ from: p.left, to: p.right, durationMs: 3600 })
    ...
    await animateShuttle({
      from: p.right,
      to: { x: p.right.x - 120, y: p.right.y + 240 },
      durationMs: 1900,
      drop: true,
      bounce: true,
    })
  ...
  const roundTwo = async () => {
    ...
    setRightPose('bathither1')
    await sleep(180)
    await animateShuttle({ from: p.right, to: p.left, durationMs: 3600 })
    ...
    setLeftPose('bathit1')
    await sleep(160)
    await animateShuttle({ from: p.left, to: p.right, durationMs: 3500 })
    ...
    await animateShuttle({
      from: p.right,
      to: { x: p.left.x + 130, y: p.left.y + 230 },
      durationMs: 1900,
      drop: true,
      bounce: true,
    })
  ...
  const onForward = () => {
    ...
    if (stage === 'r1_dialog_1') {
      setSpeaker('boy')
      setDialogue('Parunga evlo seriche kavutherera 😄')
    ...
    if (stage === 'r2_pre') {
      setSpeaker('her')
      setDialogue('Mr Paramasivan haa anga erukaru 😌')
  ...
  const phaseFiveCenterImage = phaseFiveStopMode
    ? phaseFiveStopStep === 0
      ? '/projectimages/sorry.png'
      : '/projectimages/smile.png'
    : phaseFiveIndex <= 3
    ? '/projectimages/naa.png'
    : phaseFiveIndex === 4
    ? '/projectimages/hismile.png'
    : phaseFiveIndex === 5 || phaseFiveIndex === 6
    ? '/projectimages/hi1.png'
    : phaseFiveIndex === 7
    ? '/projectimages/naa.png'
    : phaseFiveIndex === 8 || phaseFiveIndex === 9
    ? '/projectimages/hi.png'
  ...
  const phaseSevenImage =
    phaseSevenIndex <= 1
      ? '/projectimages/hi1.png'
      : phaseSevenIndex <= 3
      ? '/projectimages/shutup.png'
      : phaseSevenIndex === 4
      ? '/projectimages/hi1.png'
      : phaseSevenIndex === 5
      ? '/projectimages/rose.png'
      : phaseSevenIndex === 6
      ? '/projectimages/hismile.png'
      : phaseSevenIndex === 7
      ? '/projectimages/flower.png'
      : '/projectimages/manygift.png'
  ...
  return (
    ...
    >
      ...
      <button
        ...
        className="absolute right-3 top-3 z-[96] rounded-full border border-white/75 bg-white/88 px-3 py-1 text-xs font-semibold text-[#234a67] shadow-[0_8px_20px_rgba(20,44,68,0.2)] backdrop-blur sm:right-4 sm:top-4"
      >
        Next Phase
      </button>
          ...
          <img
            src={
              phaseSixMode === 'after_q1'
                ? '/projectimages/shocked.png'
                : phaseSixMode === 'yes'
                ? '/projectimages/crying.png'
                : phaseSixMode === 'no'
                ? phaseSixIndex <= 1
                  ? '/projectimages/naa.png'
                  : phaseSixIndex <= 3
      ...
      {stage === 'endingNote' ? (
        ...
        >
          ...
          <div className="relative z-[80] w-[min(90vw,420px)] rounded-[2rem] border border-white/60 bg-white/30 p-6 shadow-[0_8px_32px_rgba(31,38,135,0.15)] backdrop-blur-md">
            <div className="mx-auto mb-6 h-40 w-40 overflow-hidden rounded-full border-4 border-white/80 shadow-lg">
              <img
                src="/projectimages/us.png"
                alt="Us"
                className="h-full w-full object-cover"
                onError={(e) => {
                  e.target.src = '/projectimages/hismile.png'
                }}
              />
            </div>
              ...
              <button
                ...
                onClick={() => {
                  if (typeof window !== 'undefined') {
                    window.localStorage.setItem(ENDING_NOTE_KEY, endingInput)
                    const subject = 'A Message from My Love'
                    const body = encodeURIComponent(endingInput)
                    // Replace with your actual email address
                    window.location.href = `mailto:your_email@example.com?subject=${subject}&body=${body}`
                  }
                  setEndingSubmitted(true)
                }}
                className="group relative w-full overflow-hidden rounded-xl bg-gradient-to-r from-[#ff9a9e] to-[#fad0c4] py-3 text-lg font-bold text-white shadow-md transition-all hover:scale-[1.02] hover:shadow-lg active:scale-95"
              >
                <span className="relative z-10 flex items-center justify-center gap-2">Send to My Heart 💌</span>
      ...
      {showCenterIntro ? (
        <img
          src="/projectimages/batvoice.png"
          alt=""
      ...
      {shuttle.visible && stage !== 'phase5' && stage !== 'phase6' && stage !== 'phase7' && stage !== 'endingNote' ? (
        ...
        >
          <img
            src="/projectimages/cock.png"
            alt=""
            className="w-[clamp(56px,8vw,88px)] object-contain drop-shadow-[0_4px_10px_rgba(255,255,255,0.45)]"
          />
          {showShadow ? <div className="mx-auto mt-1 h-2 w-8 rounded-full bg-black/28 blur-[1.5px]" /> : null}
        </div>
      ) : null}
        ...
        <>
          ...
          {phaseFiveEmotionalShow ? (
            <motion.img
              ...
              exit={{ opacity: 0 }}
              src="/projectimages/emotional.png"
              alt=""
      ...
      {dialogue ? (
        <div className="absolute bottom-5 left-1/2 z-[90] w-[min(92vw,760px)] -translate-x-1/2 px-2 sm:bottom-8">
          <div
            className={`relative rounded-[1.4rem] border px-4 py-4 shadow-[0_14px_26px_rgba(13,45,76,0.2)] backdrop-blur-xl sm:px-6 sm:py-5 ${
              speaker === 'her' ? 'border-pink-200/80 bg-pink-50/86' : 'border-sky-200/80 bg-sky-50/88'
            }`}
          >
            <p
              className={`whitespace-pre-line text-center text-[1.14rem] font-semibold leading-[1.25] sm:text-[1.62rem] ${
